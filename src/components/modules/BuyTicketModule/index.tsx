...
const BuyTicketModule = async () => {
    ...
    const result = await supabase.from("branch")
    .select(`*, 
        faqs(*), 
        ticket(*,
        regular_ticket(*),
        booking_ticket(*))`)
    .eq("id", currentBranchId).single();
    const ticketList = result.data ? ticketReducer(undefined, { type: 'GROUP_TICKETS', payload: result.data.ticket }) : [];
    ...
    const refundDescription = result.data ? result.data.refund_policy_content : "";
    const regularTicketList = result.data ? (ticketList as any).Regular : [];
    const bookingTicketList = result.data ? (regularTicketList as any).Booking : [];
    const lowestTicket = getLowestPriceFromAllTickets(bookingTicketList, regularTicketList);
