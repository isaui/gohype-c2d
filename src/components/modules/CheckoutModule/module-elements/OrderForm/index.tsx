...
const ticketSchema = z.object({
  ...
  yearOfBirth: z.string().regex(/^\d{4}$/, { message: "Year of birth must be a 4-digit number." }),
  whatsappNumber: z.string().min(8, { message: "WhatsApp number must be at least 8 characters." }),
...
const OrderForm: React.FC<OrderFormProps> = ({ticketName="", ticketId="", ticketPath=""}) => {

  const { ticketData, totalPrice, setTotalPrice, customerData, setCustomerData, isInitialized } = useTicketStore()
  ...
  const initializeForm = () => {
    ...
    if (ticketData.count > 0) {
      ...
      if (customerData.length > 0) {
        for (let i = 0; i < Math.min(ticketData.count, customerData.length); i++) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    if(totalPrice <= 0){
      ...
      if(userRes.user){
        ...
        const response = await supabase.from("ticket_holder").insert(ticketHolders as any)
        if(!response.error){
    ...
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="p-0 space-y-1 w-full">
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="flex flex-col w-full bg-white px-4 md:px-0 py-4 md:py-0 mb-4">
            <h3 className="text-primary font-semibold mb-4">Ticket {index + 1}</h3>
        ...
        <div className="flex flex-col w-full px-4 md:px-0 md:flex-row md:justify-between md:items-start bg-white py-4">
              ...
              <AccordionItem className="no-underline p-0 hover:no-underline border-0" value="item-1">
                ...
                <AccordionContent className="hover:no-underline">
                  <div className="flex flex-col space-y-1 mt-2">
                    <Label className="text-secondary">{ticketData.count} x {ticketName}</Label>
                  </div>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
          ...
          <div className="grid md:flex mt-1">
            <Button type="submit" className="bg-primary md:ml-2 w-full">{totalPrice == 0? 'Create Order': 'Proceed to Payment'}</Button>
          </div>
        </div>
      </form>
    </Form>
